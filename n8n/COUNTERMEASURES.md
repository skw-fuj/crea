...
| **The WhatsApp bot number gets banned or restricted** | WAHA's session status stops returning `WORKING`, or re-pairing fails repeatedly | self-check flags it, alerts fire — nothing automatic recovers a banned number | this is a real possibility, not hypothetical — the bot number connects the same unofficial way WhatsApp Web does (see `INSTALL.md`). Get a new number (a cheap spare SIM works well, exactly as when you first set this up), `./go-live.sh --qr` to re-pair, no other config changes needed. Nothing here requires anyone but you |
| **The free LLM tier (Groq/OmniRoute) is discontinued or starts requiring payment** | No special detection — it looks like any other sustained LLM outage: `crea-llm`'s circuit stays open, bookings keep working via the deterministic qualifier, but it never tells you *why* beyond generic errors | Nothing breaks — CREA degrades to the fixed 5-question flow indefinitely rather than failing, so no enquiry is ever lost | If the self-check alerts or `crea-llm`'s errors persist for days rather than minutes, that's the signal — a real outage clears in minutes, a killed free tier doesn't. Check your provider's dashboard directly (console.groq.com or wherever `CREA_OMNIROUTE_URL` points), then swap to a new free tier or a cheap paid one — one config change in `config.env`, no code change (`OPERATIONS.md` §5, "Use a different / better LLM") |
